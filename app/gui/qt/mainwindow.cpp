...
#include "eric_logging/EricLogWorker.h"
...
#endif
{
    ...
    printAsciiArtLogo();
    logWorker = new EricLogWorker;
    //requestSubjectID();
    logJsonVector = new QVector<QString>;
    //registerRoutineUpload();
    ...
    // TODO: Need to design two system
    setupWindowStructure();
    createStatusBar();
    createInfoPane();
    setWindowTitle(tr("Sonic Pi"));

    // TODO: Block might not need shortcuts
    createShortcuts();
    // TODO: Need to design two system
    createToolBar();
...
void MainWindow::initPaths() {
    ...
    logging_error_log_path = QDir::toNativeSeparators(log_path + "/logging-error.log");
...
void MainWindow::setupWindowStructure() {
    ...
    errorPane->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(errorPane, SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(panel_context_menu(QPoint)));
    ...
    for(int ws = 0; ws < workspace_max; ws++) {
        ...
        QShortcut* deleteSc = new QShortcut(ctrlKey('d'), workspace, nullptr, nullptr, Qt::WidgetShortcut);
        connect(deleteSc, SIGNAL(activated()), this, SLOT(logged_delete()));
        
        // construct right click menu
        QMenu *rightClickMenu = new QMenu(tr("SonicPiScintilla Right Click Menu"), this);

        QAction* undoAct = new QAction(tr("Undo"), this);
        QShortcut* undoSc = new QShortcut(ctrlKey('z'), workspace, nullptr, nullptr, Qt::WidgetShortcut);
        updateAction(undoAct, undoSc, tr("Undo"), tr("Undo"));
        connect(undoSc, SIGNAL(activated()), this, SLOT(logged_undo()));
        connect(undoAct, SIGNAL(triggered()), this, SLOT(logged_undo()));
        rightClickMenu->addAction(undoAct);

        QAction* redoAct = new QAction(tr("Redo"), this);
        QShortcut* redoSc = new QShortcut(ctrlKey('y'), workspace, nullptr, nullptr, Qt::WidgetShortcut);
        updateAction(redoAct, redoSc, tr("Redo"), tr("Redo"));
        connect(redoSc, SIGNAL(activated()), this, SLOT(logged_redo()));
        connect(redoAct, SIGNAL(triggered()), this, SLOT(logged_redo()));
        rightClickMenu->addAction(redoAct);

        rightClickMenu->addSeparator();

        QAction* cutAct = new QAction(tr("Cut"), this);
        QShortcut* cutSc = new QShortcut(ctrlKey('x'), workspace, nullptr, nullptr, Qt::WidgetShortcut);
        updateAction(cutAct, cutSc, tr("Cut"), tr("Cut"));
        connect(cutSc, SIGNAL(activated()), this, SLOT(logged_cut()));
        connect(cutAct, SIGNAL(triggered()), this, SLOT(logged_cut()));
        rightClickMenu->addAction(cutAct);
        
        QAction* cpAct = new QAction(tr("Copy"), workspace);
        QShortcut* cpSc = new QShortcut(ctrlKey('c'), workspace, nullptr, nullptr, Qt::WidgetShortcut);
        updateAction(cpAct, cpSc, tr("Copy"), tr("Copy"));
        connect(cpSc, SIGNAL(activated()), this, SLOT(logged_copy()));
        connect(cpAct, SIGNAL(triggered()), this, SLOT(logged_copy()));
        rightClickMenu->addAction(cpAct);
        
        QAction* pasteAct = new QAction(tr("Paste"), workspace);
        QShortcut* pasteSc = new QShortcut(ctrlKey('v'), workspace, nullptr, nullptr, Qt::WidgetShortcut);
        updateAction(pasteAct, pasteSc, tr("Paste"), tr("Paste"));
        connect(pasteSc, SIGNAL(activated()), this, SLOT(logged_paste()));
        connect(pasteAct, SIGNAL(triggered()), this, SLOT(logged_paste()));
        rightClickMenu->addAction(pasteAct);

        rightClickMenu->addSeparator();

        QAction* selectAllAct = new QAction(tr("Select All"), workspace);
        QShortcut* selectAllSc = new QShortcut(ctrlKey('a'), workspace, nullptr, nullptr, Qt::WidgetShortcut);
        updateAction(selectAllAct, pasteSc, tr("Select All"), tr("Select All"));
        connect(selectAllSc, SIGNAL(activated()), this, SLOT(logged_select_all()));
        connect(selectAllAct, SIGNAL(triggered()), this, SLOT(logged_select_all()));
        rightClickMenu->addAction(selectAllAct);

        workspace->rightClickMenu = rightClickMenu;

        QString w = QString(tr("| %1 |")).arg(QString::number(ws));
        workspaces[ws] = workspace;
        tabs->addTab(workspace, w);
    }
    connect(tabs, SIGNAL(currentChanged(int)), this, SLOT(workspace_change()));
    ...
    docPane->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(docPane, SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(panel_context_menu(QPoint)));
    ...
    // docWidget is the whole resource.
    // docWidget -> dicsplit -> docPane + docCentral
...
void MainWindow::startupError(QString msg) {
    ...
    if(homeDirWritable) {
        ...
        logging_error_log = readFile(logging_error_log_path);
    }
    else {
        ...
        logging_error_log = "Permissions error: unable to access log";
    }
...
void MainWindow::showError(QString msg)
{
    ...
    focusErrors();
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    if (msg.contains("GUI Error", Qt::CaseInsensitive))
    {
        logJsonVector->append(initJson("Compile.Error", ws->text(), "GUI Error", msg.replace("&#39", "`")));
    }
    else if (msg.contains("Runtime Error", Qt::CaseInsensitive))
    {
        logJsonVector->append(initJson("Compile.Error", ws->text(), "Runtime Error", msg.replace("&#39", "`")));
    }
    else if (msg.contains("Syntax Error", Qt::CaseInsensitive))
    {
        logJsonVector->append(initJson("Compile.Error", ws->text(), "Syntax Error", msg.replace("&#39","`")));
    }
}

void MainWindow::showBufferCapacityError() {
    QString msg = "<h2 class=\"syntax_error_description\"><pre>GUI Error: Buffer Full</pre></h2><pre class=\"error_msg\"> Your code buffer has reached capacity. <br/> Please remove some code before continuing. <br/><span class=\"error_line\"> For working with very large buffers use: <br/> run_file \"/path/to/buffer.rb\"</span></pre>";
    showError(msg);
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    logJsonVector->append(initJson("Compile.Error", ws->text(), "GUI Error", msg.replace("&#39", "`")));
}
...
void MainWindow::runCode()
{
    ...
    logJsonVector->append(initJson("Run.Program", ws->text()));
    mySleep(2000);
    qDebug() << *logJsonVector;
    onetimeUpload(logJsonVector);
}
...
void MainWindow::stopCode()
{
    ...
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    logJsonVector->append(initJson("X-Stop.Program", ws->text()));
}
...
void MainWindow::loadFile(const QString &fileName, SonicPiScintilla* &text)
{
    ...
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    logJsonVector->append(initJson("File.Open", ws->text()));
    qDebug() << "MainWindow::loadFile" << *logJsonVector;
}
...
bool MainWindow::saveFile(const QString &fileName, SonicPiScintilla* text)
{
    ...
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    logJsonVector->append(initJson("File.Save", ws->text()));
    qDebug() << "MainWindow::saveFile" << *logJsonVector;
    return true;
}
...
void MainWindow::updateDocPane2(QListWidgetItem *cur, QListWidgetItem *prev) {
    ...
    updateDocPane(cur);
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    logJsonVector->append(initJson("ResourcePanel.View", ws->text(), "nan", "nan", "nan", cur->text()));
    qDebug() << "MainWindow::updateDocPane2 - ResourcePanel.View" << *logJsonVector;
}
...
void MainWindow::addUniversalCopyShortcuts(QTextEdit *te){
    new QShortcut(ctrlKey('c'), te, SLOT(copy()), nullptr,Qt::WidgetShortcut);
    new QShortcut(ctrlKey('a'), te, SLOT(selectAll()), nullptr, Qt::WidgetShortcut);
...
{
    if(obj==qApp && ( evt->type() == QEvent::ApplicationStateChange ))
    {
        statusBar()->showMessage(tr("Welcome back. Now get your live code on..."), 2000);
        update();
        
        SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
        QApplicationStateChangeEvent* qsce = static_cast<QApplicationStateChangeEvent*>(evt);
        if (qsce->applicationState() == Qt::ApplicationActive)
        {
            logJsonVector->append(initJson("X-ApplicationActive", ws->text(), "nan", "nan", "nan"));
            qDebug() << "MainWindow::eventFilter ApplicationActive: " << *logJsonVector;
        }
        else if (qsce->applicationState() == Qt::ApplicationInactive)
        {
            logJsonVector->append(initJson("X-ApplicationInactive", ws->text(), "nan", "nan", "nan"));
            qDebug() << "MainWindow::eventFilter ApplicationInactive: " << *logJsonVector;
        }
    }
    
    // shortcut event for panels
    if (evt->type() == QEvent::Shortcut)
    {
         qDebug() << "shortcut obj: " << obj;
         SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
         QShortcutEvent *sc = static_cast<QShortcutEvent *>(evt);
         const QKeySequence &ks = sc->key();
         qDebug() << "focus: " << qApp->focusWidget() << "Key Shortcut: " << ks.toString();
         if (ks.toString() == "Ctrl+C")
         {
             if (qApp->focusWidget() == errorPane)
             {
                 logJsonVector->append(initJson("ErrorPanel.Copy", ws->text(), "nan", "nan", "nan", "nan", errorPane->textCursor().selectedText().replace("&#39", "`")));
                 qDebug() << "MainWindow::eventFilter - errorPanel copy: " << *logJsonVector;
             }
             else if (qApp->focusWidget() == docPane)
             {
                 logJsonVector->append(initJson("ResourcePanel.Copy", ws->text(), "nan", "nan", "nan", "nan", docPane->textCursor().selectedText().replace("&#39", "`")));
                 qDebug() << "MainWindow::eventFilter - docPane copy: " << *logJsonVector;
             }
         }
         else if (ks.toString() == "Ctrl+A")
         {
             if (qApp->focusWidget() == errorPane)
             {
                 logJsonVector->append(initJson("ErrorPanel.SelectAll", ws->text(), "nan", "nan", "nan"));
                 qDebug() << "MainWindow::eventFilter - docPane selectall: " << *logJsonVector;
             }
             else if (qApp->focusWidget() == docPane)
             {
                 logJsonVector->append(initJson("ResourcePanel.SelectAll", ws->text(), "nan", "nan", "nan"));
                 qDebug() << "MainWindow::eventFilter - docPane selectall: " << *logJsonVector;
             }
         }
    }
    
    // key press event for workspace
    if (evt->type() == QEvent::KeyPress) 
    {
        QKeyEvent *keyEvent = static_cast<QKeyEvent *>(evt);
        if (obj->objectName().contains("Buffer "))
        {
            SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
            if (keyEvent->key() == Qt::Key_Backspace || keyEvent->key() == Qt::Key_Delete)
            {
                if (ws->hasSelectedText())
                {
                    qDebug() << "deletemulti" << ws->selectedText();
                    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "DeleteMulti", "nan", "nan", ws->selectedText()));
                }
                else
                {
                    int delline = -1;
                    int delindex = -1;
                    ws->getCursorPosition(&delline, &delindex);
                    if (keyEvent->key() == Qt::Key_Delete)
                    {
                        delindex = delindex + 1;
                    }
                    qDebug() << "deletesingle" << delindex << " " << ws->text(delline).at(delindex - 1);
                    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "DeleteSingle", "nan", "nan", ws->text(delline).at(delindex - 1)));
                }
                
                qDebug() << "obj.objectname: " << obj->objectName() << "keyEvent.text: " << keyEvent->text() << "keyEvent.key: " << keyEvent->key();
                qDebug() << *logJsonVector;
            }
            else if (keyEvent->key() >= Qt::Key_Space && keyEvent->key() <= Qt::Key_BraceRight)
            {
                logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "Insert", "nan", "nan", keyEvent->text()));
                qDebug() << "obj.objectname: " << obj->objectName() << "keyEvent.text: " << keyEvent->text() << "keyEvent.key: " << keyEvent->key();
            }
            else
            {
                qDebug() << "out!  obj.objectname: " << obj->objectName() << "keyEvent.text: " << keyEvent->text() << "keyEvent.key: " << keyEvent->key();
            }

        }
    }

    // Mouse event for panels
    if (obj->objectName() == "qt_scrollarea_viewport")
    {
        if (obj->parent() == errorPane)
        {
            SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
            if (evt->type() == QEvent::MouseButtonPress)
            {
                qDebug() << "MainWindow::eventFilter errorPane QEvent::MouseButtonPress";
                logJsonVector->append(initJson("X-ErrorPanelMouseButtonPress", ws->text(), "nan", "nan", "nan"));
            }
            else if (evt->type() == QEvent::Wheel)
            {
                qDebug() << "MainWindow::eventFilter errorPane QEvent::Wheel";
                logJsonVector->append(initJson("X-ErrorPanelMouseWheel", ws->text(), "nan", "nan", "nan"));
            } 
        }
        else if (obj->parent() == docPane)
        {
            SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
            if (evt->type() == QEvent::MouseButtonPress)
            {
                qDebug() << "MainWindow::eventFilter docPane QEvent::MouseButtonPress";
                logJsonVector->append(initJson("X-ResourcePanelMouseButtonPress", ws->text()));
            }
            else if (evt->type() == QEvent::Wheel)
            {
                qDebug() << "MainWindow::eventFilter docPane QEvent::Wheel";
                logJsonVector->append(initJson("X-ResourcePanelMouseWheel", ws->text()));
                qDebug() << "Size of the vector is " << logJsonVector->size() << ", " << *logJsonVector;
            }
        }
        else if (obj->parent() == docsCentral)
        {
            qDebug() << "panel: " << obj->parent();
        }
    }

    /*
    if (obj == errorPane && (evt->type() == QEvent::MouseButtonPress))
    {
       qDebug() << "MainWindow::eventFilter errorPane QEvent::MouseButtonPress";
    }
    */
    //if (obj == errorPane && (evt->type() == QEvent::Wheel))
    //{
     //   qDebug() << "MainWindow::eventFilter error pane wheel";
    //}
...
void MainWindow::ericLoggingError(QString msg)
{
    splashClose();
    setMessageBoxStyle();
    QString gui_log;
    ...
    QMessageBox* box = new QMessageBox(QMessageBox::Warning,
        tr("Logging boot error..."), tr("Sonic Pi Boot Error\n\nApologies, a critical error occurred during startup for logging") + ":\n\n " + msg + "\n\n" + tr("Please consider reporting a bug to ericzh@udel.edu"));
    QString error_report = "Sonic Pi Logging Boot Error Report\n==================\n\n\nSystem Information\n----------------\n\n* Sonic Pi version: " + version + "\n* OS: " + osDescription() + "\n\n\nLogging Error\n-------\n\n**`" + logging_error_log_path + "`**\n```\n" + logging_error_log + "\n\n\nGUI Log\n-------\n\n**`" + gui_log_path + "`**\n```\n" + gui_log + "\n```\n\n\nServer Errors\n-------------\n\n**`" + server_error_log_path + "`**\n```\n" + server_error_log + "\n```\n\n\nServer Output\n-------------\n\n**`" + server_output_log_path + "`**\n```\n" + server_output_log + "\n```\n\n\nScsynth Output\n--------------\n\n**`" + scsynth_log_path + "`**\n```\n" + scsynth_log + "\n```\n\n\nProcess Log\n--------------\n\n**`" + process_log_path + "`**\n```\n" + processes_log + "\n\n\n```\n";
    box->setDetailedText(error_report);
...
QString MainWindow::initJson(QString eventType, QString code, QString compileMsgType, QString compileMsgData, QString editType, QString resourceViewTitle, QString clipboardContent, QString editContent)
{
    QJsonObject json;
    json.insert("SubjectID", subjectID);

    json.insert("EventType", eventType);
#if defined(Q_OS_MAC)
    json.insert("ToolInstances", "Sonic Pi 3.2.2 Mac");
#else
    json.insert("ToolInstances", "Sonic Pi 3.2.2 Win");
#endif

    QDateTime time = QDateTime::currentDateTime();
    int timeT = time.toTime_t();
    json.insert("LocalTimeStamp", timeT);

    json.insert("Code", code);

    json.insert("CompileMessageType", compileMsgType);
    json.insert("CompileMessageData", compileMsgData);

    json.insert("EditType", editType);

    json.insert("ResourceViewTitle", resourceViewTitle);
    json.insert("ClipboardContent", clipboardContent);
    json.insert("EditContent", editContent);
...
void MainWindow::requestSubjectID()
{
    QThread* requestIDThread = new QThread();
    logWorker->moveToThread(requestIDThread);
    QObject::connect(requestIDThread, &QThread::started, logWorker, [=] {
        subjectID = logWorker->requestPost("request_id", "1", false);
        qDebug() << "subject ID: " << subjectID;
    });
    QObject::connect(logWorker, SIGNAL(finished()), requestIDThread, SLOT(quit()));
    QObject::connect(requestIDThread, SIGNAL(finished()), requestIDThread, SLOT(deleteLater()));
    requestIDThread->start();
}

void MainWindow::onetimeUpload(QVector<QString>* uploadJSONVector)
{
    if (!uploadJSONVector->isEmpty())
    {
        QString queryCount = logWorker->uploadLog(*uploadJSONVector);
        qDebug() << "MainWindow::onetimeUpload, query count: " << queryCount;
        if (queryCount == "-1")
        {
            qDebug() << "MainWindow::onetimeUpload, failed -1";
        }
        else
        {
            if (!uploadJSONVector->isEmpty())
            {
                uploadJSONVector->remove(0, queryCount.toInt());
                qDebug() << "MainWindow::onetimeUpload, Delete vector elements " << queryCount;
            }
            else
            {
                qDebug() << "MainWindow::onetimeUpload, Caution, vector is already empty! ";
            }
        }
        if (uploadJSONVector->isEmpty())
        {
            qDebug() << "MainWindow::onetimeUpload, after upload, uploadJSONVector empty";
        }
        else
        {
            qDebug() << "MainWindow::onetimeUpload, after upload, uploadJSONVector not empty\n"
                     << *uploadJSONVector;
        }
    }
    else
    {
        qDebug() << "logVector is empty";
    }
}

void MainWindow::mySleep(int msec) 
{
...
void MainWindow::logged_undo()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    ws->sp_undo();
    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "Undo"));
    qDebug() << "MainWindow::logged_undo(): " << *logJsonVector;
}

void MainWindow::logged_redo()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    ws->sp_redo();
    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "Redo"));
    qDebug() << "MainWindow::logged_redo(): " << *logJsonVector;
}

void MainWindow::logged_cut()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "X-Cut", "nan", ws->selectedText()));
    ws->sp_cut();
    qDebug() << "MainWindow::logged_cut(): " << *logJsonVector;
}

void MainWindow::logged_copy()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "X-Copy", "nan", ws->selectedText()));
    ws->sp_copy();
    qDebug() << "MainWindow::logged_copy(): " << *logJsonVector;
}

void MainWindow::logged_paste()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();

    int* lineFrom = new int;
    int* indexFrom = new int;
    ws->getCursorPosition(lineFrom, indexFrom);

    ws->sp_paste();

    int* lineTo = new int;
    int* indexTo = new int;
    ws->getCursorPosition(lineTo, indexTo);
    
    ws->setSelection(*lineFrom, *indexFrom, *lineTo, *indexTo);
    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "X-Paste","nan", "nan", ws->selectedText()));
    ws->deselect();

    qDebug() << "MainWindow::logged_paste(): " << *logJsonVector;
}

void MainWindow::logged_select_all()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    ws->sp_select_all();
    logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "X-SelectAll"));
    qDebug() << "MainWindow::logged_select_all(): " << *logJsonVector;
}

void MainWindow::logged_delete()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    if (ws->hasSelectedText()) 
    {
    ...
    else
    {
        ...
        int delindex = -1;
        ws->getCursorPosition(&delline, &delindex);
        qDebug() << "deletesingle" << ws->text(delline)[delindex - 1];
        logJsonVector->append(initJson("File.Edit", ws->text(), "nan", "nan", "DeleteSingle", "nan", "nan", ws->text(delline).at(delindex - 1)));
    }
    ws->sp_delete();
    qDebug() << "MainWindow::logged_delete(): " << *logJsonVector;
}

void MainWindow::panel_context_menu(QPoint) 
{
    QMenu* menu = new QMenu;
    QString appStyling = theme->getAppStylesheet();
    menu->setStyleSheet(appStyling);


    QAction* CpAct = new QAction("Copy", this);
    CpAct->setText("Copy\t" + ctrlKey('c').toString(QKeySequence::NativeText));
    connect(CpAct, SIGNAL(triggered()), this, SLOT(logged_panel_copy()));
    menu->addAction(CpAct);

    QAction* SelectAllAct = new QAction("Select All", this);
    SelectAllAct->setText("Select All\t" + ctrlKey('a').toString(QKeySequence::NativeText));
    connect(SelectAllAct, SIGNAL(triggered()), this, SLOT(logged_panel_select_all()));
...
void MainWindow::logged_panel_copy()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    qDebug() << "MainWindow::logged_panel_copy() " << this->focusWidget();
    if (this->focusWidget() == errorPane)
    {
        errorPane->copy();
        logJsonVector->append(initJson("ErrorPanel.Copy", ws->text(), "nan", "nan", "nan", "nan", errorPane->textCursor().selectedText().replace("&#39", "`")));
        qDebug() << "MainWindow::logged_panel_copy() - errorPanel copy" << *logJsonVector;
    }
    else if (this->focusWidget() == docPane)
    {
        docPane->copy();
        logJsonVector->append(initJson("ResourcePanel.Copy", ws->text(), "nan", "nan", "nan", "nan", docPane->textCursor().selectedText().replace("&#39", "`")));
        qDebug() << "MainWindow::logged_panel_copy() - docPane copy" << *logJsonVector;
    }
}


void MainWindow::logged_panel_select_all()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    qDebug() << "MainWindow::logged_panel_select_all() " << this->focusWidget();
    if (this->focusWidget() == errorPane)
    {
        errorPane->selectAll();
        logJsonVector->append(initJson("ErrorPanel.SelectAll", ws->text()));
        qDebug() << "MainWindow::logged_panel_select_all() - errorPanel selectall" << *logJsonVector;
    }
    else if (this->focusWidget() == docPane)
    {
        docPane->selectAll();
        logJsonVector->append(initJson("ResourcePanel.SelectAll", ws->text()));
        qDebug() << "MainWindow::logged_panel_select_all() - docPane selectall" << *logJsonVector;
    }
}

void MainWindow::workspace_change()
{
    SonicPiScintilla* ws = (SonicPiScintilla*)tabs->currentWidget();
    int index = tabs->currentIndex();
    QString eventText = "X-Workspace.ChangeTo";
    logJsonVector->append(initJson(eventText.append(QString::number(index)), ws->text()));
    qDebug() << "MainWindow::workspace_change()" << *logJsonVector;
}


void MainWindow::registerRoutineUpload()
{
    EricLogWorker *ericLogWorker = new EricLogWorker();
    QTimer* timer = new QTimer();
    connect(timer, &QTimer::timeout, ericLogWorker, [=] {
        onetimeUpload(logJsonVector);
        });
    connect(ericLogWorker, SIGNAL(finished()), ericLogWorker, SLOT(deleteLater()));
    timer->start(90 * 1000);
}